...
void serialEvent1(){
  while (Serial1.available()) {
   ...
   // Serial.print("POLLO:"+String(inChar));
    FifoPush( &FifoRx, inChar );
   }
}
...
void at_send_command(char *command) 
{
  ...
  sprintf(command_echo,"%s\r\r\n",command);

  if (at_echo_mode)
  {
    if ( at_read_for_response_single( command_echo, 2000, 0 ) != 0 )
      Serial.println("*** ECHO ERROR***");
  }
}

uint8_t at_read_for_response_single( char* response_string, uint32_t timeout_ms, uint8_t echo) 
{
...
uint8_t read_for_responses_dual(char* pass_string, char* fail_string, uint32_t timeout_ms, uint8_t echo) 
{
